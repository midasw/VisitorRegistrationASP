...
namespace VisitorRegistration.MVC.Areas.Manage.Controllers
{
    ...
    public class CompaniesController : BaseController
    {
        ...
        public CompaniesController(ICompanyService service, UserOptions userOptions, IStringLocalizer<SharedResources> localizer)
        {
            _service = service;
        ...
        public async Task<IActionResult> Create([Bind("Name")] Company company)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    StatusMessage = Localizer["Successfully created one company"];
        ...
        public async Task<IActionResult> Edit(int id, [Bind("Id,Name")] Company company)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    StatusMessage = Localizer["Successfully updated one company"];
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            if (result.Succeeded)
            {
                StatusMessage = Localizer["Successfully deleted one company"];
            }
        ...
        public async Task<IActionResult> DeleteMultiple(List<int> checkedIds, bool confirmed = false)
        {
            ...
            if (confirmed)
            {
                ...
                if (result.Succeeded)
                {
                    StatusMessage = string.Format(Localizer["Successfully deleted {0} companies"], result.AffectedRows);
                }
